...
[_] QuardSPI
    ...
    [_] Собственная реализация методом копи-пасты с необходимыми модицикациями
        ...
        [v] Конфигурирование флешки на чтение в режиме датчика
            ...
            - Чтение работает, но хост гугается на таймауты и в итоге флешку не монтирует.
...
[_] Обнаружен баг: С некоторой вероятностью блиотека SCSI может отправлять результат чтения хосту
    длиной меньше запрошенного. Замечен недостаток, кратный 128 байтам, возможно меньше.
    Сами данные корректные, только часть не присылается.
    ...
    Процессор загружен высоко процессом USB, Снизил приоритет чтобы не ломалась измериловка.
        [_] Почему теряется хвост пакета?