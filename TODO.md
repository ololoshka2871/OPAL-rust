...
[v] Тестирование энергопоребления и разрешения на разных опорных частотах и с разными кварцами
    `Тестирование потребления самописца.xlsx`
    [v] Режим датчика: оценить влияние частоты опорного кварцевого резонатора на разрешение частотомера
        Есть ли разница, использовать резонатор 12MHz или 24Mhz
        [v] Создать фичу cargo, в которой бы задавалась частота опоры
        [v] Настроить соответственно 2 варианта конфигурации клоков в коде
        [v] Провезти измерения
    [v] Режим самописца: Оценить влияние опорной частоты на разрешение и энергопоребление
        [v] Создать фичу cargo, которая задяет режим работы самописца recorder-power-save, recorder-balanced, 
            recorder-performance.
        [v] Настроить соответственно 3 варианта конфигурации клоков в коде
        [v] Провезти измерения
        
[v] Обновлены версии зависимостей, тест стабильности.

[_] Проверка ввода опорной частоты, +/- от базы а не от текущего