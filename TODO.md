...
[v] Частотомеры
    ...
    [_] Обнаружена иголка. Значение result: 19991648 -> 200316 = diff|19791332, затем адаптация воостановила.
    ...
    [v] Частота зависит от времени измерения?
        ...
        [_] Таргет давления не опускается ниже 99 при времени измерения < 100 мс
        [_] При малых временах измерения канал давления вечно теряет частоту на входе
    [v] DMA
...
[_] Режим самописца
    ...
    [_] Архитектура
        ...
        [_] Отключение генераторов при большой разнице между временем измерения и периодом записи
    [_] Запись
        [_] Подготовка данных
        [_] Сжатие страниц
        [_] Запись во flash
    [_] Адаптация
        [_] В случае перехода на новую страницу
        [_] В случае запуска после сна
        [_] Автоадаптация тебует по крайней мере 2 цыкла для получения правильного значения target
...
[x] Стандартные Vid/Pid чтобы windows7 распознавала CDC/ACM
    ...
    [_] Сделать одной функцией любые статические данныне, не только строки
[_] Утечка памяти при сохранении настроек - краш после пары раз
