...
[_] Экономия процессорного времени
    [_] Пробуждать поток protobuf только если usb.pool() == true
    [_] Подбор переиода сна usbd
...
[_] Частотомеры
    ...
    [_] Частотмер на Rust
        ...
        [_] Мастер-счетчик
            ...
            [_] Маестер счетчик 32 бита (16+16), соответственно он должен преполняться с периодом 
                1/80кк * 2^32 = 53,6 c
            [_] Снизить базовую частоту делителем APB1 (/8) (может меньше из-за USB)
            [_] Низкий приоритет прерывания мастер-счетчика
                ```
