...
[_] Режим самописца
    ...
    [_] "Засыпание" в случае большого интервала записи
        ...
        [_] Канал температуры показывет неадекватное значение - адаптация не проходит
            [_] Запускать канала в 2+ этапа, чтобы полцчить минимальную задержку после включения питания
        [_] Записать 2 страницы в таком режиме и проверить корректность
        [_] A АЦП выключается между шапками?
