...
[_] Пониженное энергопоребление
    ...
    [_] Обычный режим процессора - Low-power run
        [v] Есть толк
        [_] Снизить частоту процессора, переписсать cfgr.freeze() под конкретные задачи для обоих режимов
